...
def open_file2():
    name = tf.get()
    # Pass inputs as arguments when opening file2
    exec(open("p1.py").read(), {'name': name})
...
# Fetch button
fetch_button = customtkinter.CTkButton(f1_2, text="Fetch Students", command=fetch_students,fg_color='#0C2D57',width=343,height=40,state='disable')
fetch_button.place(relx=0.03,rely=0.76)

voice_but = customtkinter.CTkButton(f2, text="VOICE",command = speak_text,fg_color='#0C2D57',width=200,height=40)
